...
description: >-
  ...
  payouts for SourceByJay Phase 10. Use when implementing checkout, vendor
  onboarding commission, webhooks, or application fees. Do not store card data.
---

# SourceByJay Payments (Phase 10)

**Never store card numbers.** Use Stripe Connect only.

## Until Stripe (Phase 3+ — locked)

Use the **fake payment slab** so you can test RFQ → quote → order → status without Stripe bottlenecks:

- `payments.provider = 'fake'`
- Buyer or ops clicks **Mark paid (test)** → `fake_mark_order_paid`
- UI must say TEST MODE / not real money
- Swap to Stripe in Phase 10 (`provider = 'stripe'`) when Owner is ready

See DECISIONS.md → Payments.
...
suppliers.stripe_connect_account_id  -- Phase 10
```
...
Do not start Phase 10 before Phases S0–9 and user GO on payment scope.
